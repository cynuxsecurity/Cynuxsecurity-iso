# paru
# Autogenerated from man page /usr/lib/jvm/default/man/man8/paru.8.gz
...
complete -c paru -l local -d 'Paru will also remove the package from the local repo'
...
complete -c paru -s d -l delete -d 'Remove a package from the local repo'
complete -c paru -s q -l quiet -d 'Show less information'
complete -c paru -s i -l install -d 'Install a package into the chroot'
complete -c paru -s u -l upgrade -d 'Upgrade the chroot'
...
complete -c paru -l builddir -d 'Directory to use for building AUR Packages'
...
complete -c paru -l nosudoloop -d 'Do not loop sudo calls in the background.  localrepo [= Repos'
complete -c paru -l chroot -d 'Build packages in a chroot.  This rquires the LocalRepo option to be enabled'
complete -c paru -l nochroot -d 'Don\'t build packages in a chroot.  EXAMPLES'
